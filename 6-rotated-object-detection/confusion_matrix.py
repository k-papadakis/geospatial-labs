...
# Modified by k.i.papadakis@gmail.com
import argparse
import os
...
def parse_args():
    ...
    parser.add_argument(
        '--title',
        default=None,
        help='The title of the confusion matrix'
    )
...
def alt_plot_confusion_matrix(
    ...
    confusion_matrix = confusion_matrix / per_label_sums
    confusion_matrix[np.isnan(confusion_matrix)] = -1
    
    num_classes = len(labels)
    fig, ax = plt.subplots(
        figsize=(0.7 * num_classes, num_classes * 0.7))
    sns.heatmap(
        confusion_matrix,
        cmap=color_theme,
        annot=True,
...
def main():
    ...
    classes = [c.replace('-', ' ').title() for c in dataset.CLASSES + ('background',)]
    
    alt_plot_confusion_matrix(
        confusion_matrix,
        classes,
